use super::{ChunkPos, SEA_LEVEL_PERCENT, TilePos, WORLD_REGION_SIZE};
...
impl BiomeMap {
    ...
    pub(crate) fn generate(seed: u64, width: u32, height: u32) -> Self {
        let chunks_wide = width.div_ceil(WORLD_REGION_SIZE);
        let chunks_high = height.div_ceil(WORLD_REGION_SIZE);
        ...
        let surface_chunk = (height * SEA_LEVEL_PERCENT / 100) / WORLD_REGION_SIZE;
...
impl super::World {
    ...
    pub fn biome_at(&self, position: TilePos) -> Option<BiomeId> {
        ...
        self.biome_in_chunk(ChunkPos {
            x: position.x / WORLD_REGION_SIZE,
            y: position.y / WORLD_REGION_SIZE,
        })
    }
...
mod tests {
    ...
    fn generated_crystal_region_is_deterministic_and_underground() {
        ...
        let first_underground = (1_024 * SEA_LEVEL_PERCENT / 100) / WORLD_REGION_SIZE + 1;

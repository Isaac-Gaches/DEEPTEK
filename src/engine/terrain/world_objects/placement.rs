use super::*;
use crate::ItemTransportRole;
use crate::terrain::NaturalObject;
...
impl World {
    pub fn place_natural_object(
        ...
        let variant = if object_type == NaturalObject::GRASS {
            (super::super::nature::natural_hash(self.seed, anchor.x, anchor.y) % 2) as u8
        } else {
            0
        };
            ...
            .filter(|object| object.object_type == ROPE_OBJECT)
        {
            ...
            loop {
                ...
                if let Some(lower) = self
                    .object_at(next)
                    .filter(|lower| lower.object_type == ROPE_OBJECT && lower.anchor == next)
                {
                    rope = lower;
                    continue;
                }
    ...
    pub fn place_or_extend_rope(
        ...
        if let Some(mut rope) = extension {
            while let Some(above) = self
                .objects
                .object(rope)
                .and_then(|object| object.anchor.y.checked_sub(1))
                .and_then(|y| self.object_at(TilePos::new(placement.x, y)))
                .filter(|above| {
                    above.object_type == ROPE_OBJECT
                        && above.anchor.y + u32::from(above.height)
                            == self.objects.object(rope).unwrap().anchor.y
                })
                .map(WorldObject::id)
            {
                rope = above;
            ...
            loop {
                let bottom = self.objects.object(rope).map(|object| {
                    TilePos::new(object.anchor.x, object.anchor.y + u32::from(object.height))
                });
                let lower = bottom.and_then(|bottom| {
                    self.object_at(bottom)
                        .filter(|object| {
                            object.object_type == ROPE_OBJECT && object.anchor == bottom
                        })
                        .map(WorldObject::id)
                });
                ...
                if !self.objects.merge_down(rope, lower) {
                    return Err(ObjectPlacementError::Occupied(placement));
                }
            }
            let changed = self.objects.extend_down(rope);
            return changed
                .then_some(placement)
    ...
    pub fn can_place_furniture(
        ...
        for y in anchor.y..=end_y {
            for x in anchor.x..=end_x {
                ...
                if self.tile_in_bounds(x, y, Layer::Foreground) != TileId::EMPTY
                    || self
                        .object_at(position)
                        .is_some_and(|object| !is_natural_object(object.object_type()))
                {
                    return Err(ObjectPlacementError::Occupied(position));
                }
            }
        ...
        if definition.behavior() == FurnitureBehavior::CargoLift {
    ...
    pub(crate) fn transfer_one_container_item(
        ...
        // Processor input slots are protected at the world boundary. Processor
        // output must use the explicit output-slot transfer path, so no generic
        // conveyor or future cargo system can accidentally extract ingredients.
        if self.object(source).is_some_and(|object| {
            furniture_definition(object.object_type()).is_some_and(|definition| {
                definition.interaction().item_transport_role() == Some(ItemTransportRole::Processor)
            })
        }) {
            return false;
        }
    ...
    pub(crate) fn remove_rope_at_and_below(
        &mut self,
        position: TilePos,
    ) -> Vec<super::RemovedObject> {
        let Some(id) = self.objects.occupying(position) else {
        ...
        self.objects.remove_rope_from(id, position.y)
    }
    ...
    pub(crate) fn blocks_foreground_tile_placement(&self, position: TilePos) -> bool {
        self.object_at(position).is_some_and(|object| {
            object.object_type() != ROPE_OBJECT && !is_natural_object(object.object_type())
        })
    }

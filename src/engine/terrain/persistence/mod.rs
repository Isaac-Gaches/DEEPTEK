...
use super::{
    ...
    SurveyedArea, TargetPriority, TileId, TilePos, World, WorldError, WorldObject,
    configuration_variant, furniture_definition, objects::ObjectStore, parallel_mut,
};
...
const SURVEYED_AREAS_VERSION: u16 = 21;
const DELAYED_MISSION_TRANSMISSIONS_VERSION: u16 = 20;
...
const MAX_SURVEYED_AREAS: usize = 4_096;
...
pub(super) fn save(world: &World, path: &Path, threads: usize) -> Result<(), WorldError> {
    ...
    let surveyed_area_payload = encode_surveyed_areas(world)?;
    ...
    let result = (|| {
        ...
        file.write_all(&(surveyed_area_payload.len() as u32).to_le_bytes())?;
        file.write_all(&checksum(&surveyed_area_payload).to_le_bytes())?;
        file.write_all(&surveyed_area_payload)?;
        file.flush()?;
        file.sync_all()?;
        drop(file);
...
pub(super) fn load(path: &Path, threads: usize) -> Result<World, WorldError> {
    ...
    if !matches!(
        ...
        LEGACY_VERSION
            ...
            | PROSPECTOR_PROGRESS_VERSION
            | DELAYED_MISSION_TRANSMISSIONS_VERSION
            | VERSION
    ) {
    ...
    let saved_chunks_wide = width.div_ceil(chunk_size as u32);
    let saved_chunks_high = height.div_ceil(chunk_size as u32);
    if chunk_count != saved_chunks_wide as usize * saved_chunks_high as usize {
        return invalid("chunk count does not match world dimensions");
    }
    let saved_chunk_area = chunk_size
        .checked_mul(chunk_size)
        .ok_or_else(|| WorldError::InvalidData("chunk area overflow".into()))?;
    ...
    for _ in 0..chunk_count {
        ...
        if foreground_len > saved_chunk_area * 4 || background_len > saved_chunk_area * 4 {
    ...
    let surveyed_area_payload = if version >= SURVEYED_AREAS_VERSION {
        let payload_len = read_u32(&bytes, &mut cursor)? as usize;
        let expected_checksum = read_u32(&bytes, &mut cursor)?;
        let payload = take(&bytes, &mut cursor, payload_len)?;
        if checksum(payload) != expected_checksum {
            return invalid("surveyed area section checksum mismatch");
        }
        Some(payload)
    } else {
        None
    ...
    if chunk_size == CHUNK_SIZE {
        ...
        let mut foreground = vec![TileId::EMPTY; saved_chunk_area];
        let mut background = vec![TileId::EMPTY; saved_chunk_area];
        for (index, record) in records.iter().enumerate() {
            let encoded_foreground = &bytes[record.foreground.clone()];
            let encoded_background = &bytes[record.background.clone()];
            if checksum(encoded_foreground) != record.foreground_checksum
                || checksum(encoded_background) != record.background_checksum
            {
                return invalid(format!("chunk {index}: checksum mismatch"));
            }
            decode_layer(encoded_foreground, &mut foreground)
                .and_then(|()| decode_layer(encoded_background, &mut background))
                .map_err(|message| WorldError::InvalidData(format!("chunk {index}: {message}")))?;
            let chunk_x = index as u32 % saved_chunks_wide;
            let chunk_y = index as u32 / saved_chunks_wide;
            ...
            for local_y in 0..valid_height {
                for local_x in 0..valid_width {
                    ...
                    let (destination, destination_x, destination_y) = world.locate(x, y)?;
                    let source = local_y * chunk_size + local_x;
                    world.chunks[destination].foreground
                        [destination_y * CHUNK_SIZE + destination_x] = foreground[source];
                    world.chunks[destination].background
                        [destination_y * CHUNK_SIZE + destination_x] = background[source];
                }
            }
        }
    }
    ...
    if let Some(payload) = surveyed_area_payload {
        decode_surveyed_areas(&mut world, payload)?;
    }
    Ok(world)
}

fn encode_surveyed_areas(world: &World) -> Result<Vec<u8>, WorldError> {
    let areas: Vec<_> = world.surveyed_areas().collect();
    if areas.len() > MAX_SURVEYED_AREAS {
        return invalid("surveyed area count exceeds its maximum");
    }
    let mut bytes = Vec::with_capacity(4 + areas.len() * 16);
    bytes.extend_from_slice(&(areas.len() as u32).to_le_bytes());
    for area in areas {
        bytes.extend_from_slice(&area.minimum.x.to_le_bytes());
        bytes.extend_from_slice(&area.minimum.y.to_le_bytes());
        bytes.extend_from_slice(&area.maximum_exclusive.x.to_le_bytes());
        bytes.extend_from_slice(&area.maximum_exclusive.y.to_le_bytes());
    }
    Ok(bytes)
}

fn decode_surveyed_areas(world: &mut World, bytes: &[u8]) -> Result<(), WorldError> {
    let mut cursor = 0;
    let count = read_u32(bytes, &mut cursor)? as usize;
    if count > MAX_SURVEYED_AREAS || bytes.len() != 4 + count * 16 {
        return invalid("invalid surveyed area section length");
    }
    let mut areas = Vec::with_capacity(count);
    for _ in 0..count {
        let minimum = TilePos::new(read_u32(bytes, &mut cursor)?, read_u32(bytes, &mut cursor)?);
        let maximum_exclusive =
            TilePos::new(read_u32(bytes, &mut cursor)?, read_u32(bytes, &mut cursor)?);
        if minimum.x >= maximum_exclusive.x
            || minimum.y >= maximum_exclusive.y
            || maximum_exclusive.x > world.width()
            || maximum_exclusive.y > world.height()
        {
            return invalid("surveyed area lies outside the world");
        }
        areas.push(SurveyedArea {
            minimum,
            maximum_exclusive,
        });
    }
    world.survey_revision = areas.len() as u64;
    world.surveyed_areas = areas;
    Ok(())
}
...
fn encode_mission_state(
    ...
    for remaining_ms in progress.package_transmission_delays_ms {
        output.extend_from_slice(&remaining_ms.to_le_bytes());
    }
...
> {
    ...
    let progress = if version >= PROSPECTOR_PROGRESS_VERSION {
        let mut progress = SavedProspectorProgress {
            ...
            package_transmission_delays_ms: [0; 5],
        };
        if version >= DELAYED_MISSION_TRANSMISSIONS_VERSION {
            for remaining_ms in &mut progress.package_transmission_delays_ms {
                *remaining_ms = read_u32(bytes, cursor)?;
            }
        }
        Some(progress)
    } else {

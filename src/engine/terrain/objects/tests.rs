...
fn chest_occupies_a_two_by_two_footprint_and_exposes_interaction() {
    ...
    assert!(matches!(
        world.set_tile(2, 3, Layer::Foreground, ForegroundTile::DIRT),
        Err(crate::WorldError::OccupiedByObject { object }) if object == id
    ));
    assert_eq!(world.object_at(TilePos::new(2, 3)).unwrap().id(), id);
    assert_eq!(world.tile_in_bounds(2, 3, Layer::Foreground), TileId::EMPTY);
}

#[test]
fn furniture_replaces_natural_decorations_in_its_footprint() {
    let mut world = chest_floor();
    let grass = world
    ...
    assert!(
        world
            .can_place_furniture(FurnitureObject::CHEST, TilePos::new(2, 3))
            .is_ok()
    );
    let chest = world
        .place_furniture(FurnitureObject::CHEST, TilePos::new(2, 3))
    ...
    assert!(world.object(grass).is_none());
    assert!(world.object(pebble).is_none());
    assert_eq!(world.object_at(TilePos::new(2, 4)).unwrap().id(), chest);
    assert_eq!(world.object_at(TilePos::new(3, 4)).unwrap().id(), chest);
}

#[test]
fn foreground_blocks_replace_natural_decorations() {
    let mut world = World::empty(8, 8, 0).unwrap();
    world
    ...
    assert!(world.can_place_tile_adjacent(TilePos::new(3, 4), Layer::Foreground));
    world
        .set_tile(3, 4, Layer::Foreground, ForegroundTile::STONE)
        .unwrap();

    assert!(world.object(pebble).is_none());
    assert_eq!(
        world.tile_in_bounds(3, 4, Layer::Foreground),
...
#[test]
fn laser_bore_occupies_four_by_three_cells_and_centres_its_open_shaft() {
    ...
    let id = world
        ...
        .unwrap();
    assert_eq!(world.object(id).unwrap().size(), [4, 3]);
    assert_eq!(
        world.furniture_interaction_at(TilePos::new(3, 4)),
        Some((
            id,
    ...
    world
        .set_tile(4, 6, Layer::Foreground, ForegroundTile::DIRT)
        .unwrap();
    assert!(world.object(id).is_some());
    world
...
fn touching_rope_objects_merge_into_one_column() {
    ...
    assert_eq!(
        world.rope_placement_target(upper_position),
        Ok(TilePos::new(3, 6))
    );
    assert_eq!(
    ...
    assert_eq!(world.object(upper).unwrap().size(), [1, 5]);
    assert_eq!(world.object_at(lower_position).unwrap().id(), upper);
    assert_eq!(world.objects_of_type(ROPE_OBJECT).count(), 1);
}

#[test]
fn cutting_rope_preserves_the_column_above_and_removes_the_rest() {
    let mut world = World::empty(8, 12, 0).unwrap();
    world
        .set_tile(3, 1, Layer::Foreground, ForegroundTile::STONE)
        .unwrap();
    let anchor = world.place_or_extend_rope(TilePos::new(3, 2)).unwrap();
    for _ in 0..4 {
        world.place_or_extend_rope(anchor).unwrap();
    }
    let rope = world.object_at(anchor).unwrap().id();

    let removed = world.remove_rope_at_and_below(TilePos::new(3, 4));

    assert_eq!(removed.len(), 1);
    assert_eq!(removed[0].object().anchor(), TilePos::new(3, 4));
    assert_eq!(removed[0].object().size(), [1, 3]);
    assert_eq!(world.object(rope).unwrap().size(), [1, 2]);
    assert_eq!(world.object_at(TilePos::new(3, 3)).unwrap().id(), rope);
    assert!(world.object_at(TilePos::new(3, 4)).is_none());
    assert_eq!(world.objects_of_type(ROPE_OBJECT).count(), 1);
}

#[test]
fn cutting_rope_removes_contiguous_legacy_sections_below_the_cut() {
    let mut world = World::empty(8, 14, 0).unwrap();
    world
        .set_tile(3, 1, Layer::Foreground, ForegroundTile::STONE)
        .unwrap();
    world
    ...
    world.place_or_extend_rope(upper_position).unwrap();
    world.place_or_extend_rope(upper_position).unwrap();
    assert_eq!(world.objects_of_type(ROPE_OBJECT).count(), 2);

    let removed = world.remove_rope_at_and_below(TilePos::new(3, 3));

    assert_eq!(removed.len(), 2);
    assert_eq!(world.object(upper).unwrap().size(), [1, 1]);
    assert!(world.object(lower).is_none());
    assert!(world.object_at(TilePos::new(3, 3)).is_none());
    assert!(world.object_at(TilePos::new(3, 5)).is_none());
}
...
fn powered_cable_carries_its_endpoints_and_hosts_one_persistent_lift() {
    ...
    world
        .set_tile(9, 7, Layer::Foreground, ForegroundTile::STONE)
        .unwrap();
    let grass = world
        .place_natural_object(
            crate::NaturalObject::GRASS,

...
pub use furniture::{
    ...
    BATTERY_CAPACITY_MILLI, BATTERY_DEFINITION, BED_DEFINITION, BUILT_IN_FURNITURE, BoreBeamVisual,
    ...
    FurnitureBehavior, FurnitureConfiguration, FurnitureDefinition, FurnitureFacing,
    FurnitureInteraction, FurnitureItem, FurnitureObject, FurnitureOffer, FurnitureSupport,
    ItemTransportRole, LASER_BORE_DEFINITION, LASER_BORE_DEMAND_MILLI_PER_SECOND,
    LASER_BORE_MAX_LENGTH, LASER_BORE_SLOTS, LASER_BORE_TICKS_PER_TILE, LASER_BORE_WIDTH,
    LASER_DRILL_DEFINITION, LASER_DRILL_DEMAND_MILLI_PER_SECOND, LASER_DRILL_MAX_LENGTH,
    LASER_DRILL_SLOTS, LIFT_STATION_DEFINITION, LIFT_STATION_SLOTS, LaserDrillAim,
    LiftStationConfiguration, LiftStationMode, ORBITAL_EXPORT_DEMAND_MILLI_PER_SECOND,
    ORBITAL_EXPORT_LAUNCHER_DEFINITION, ORBITAL_EXPORT_LAUNCHER_SLOTS,
    POWER_CONNECTION_RANGE_HALF_TILES, POWER_CONNECTION_RANGE_TILES, POWER_CONNECTOR_DEFINITION,
    POWER_CONNECTOR_RANGE_TILES, POWERED_CABLE_ANCHOR_DEFINITION, PROCUREMENT_TERMINAL_DEFINITION,
    PROCUREMENT_TERMINAL_DEMAND_MILLI_PER_SECOND, PYLON_DEFINITION, PowerRole,
    RED_SHAFT_BORE_DEFINITION, RED_SHAFT_BORE_DEMAND_MILLI_PER_SECOND, RED_SHAFT_BORE_SLOTS,
    RED_SHAFT_BORE_WIDTH, SOLAR_ARRAY_DEFINITION, SOLAR_GENERATION_MILLI_PER_SECOND,
    SPIKES_DEFINITION, SUBSURFACE_SURVEY_DEPTH, SUBSURFACE_SURVEY_WIDTH,
    SUBSURFACE_SURVEYOR_DEFINITION, SUBSURFACE_SURVEYOR_DEMAND_MILLI_PER_SECOND, TURRET_DEFINITION,
    TURRET_DEMAND_MILLI_PER_SECOND, TargetPriority, VerticalBoreBehavior, furniture_definition,
};
...
pub use survey::{MAX_SURVEY_ORE_TYPES, OreEstimate, SubsurfaceSurvey, SurveyedArea};

/// Storage and mesh partition size. Gameplay and visual tuning must use explicit
/// world-space tile distances rather than deriving distances from this value.
pub const CHUNK_SIZE: usize = 32;
const CHUNK_AREA: usize = CHUNK_SIZE * CHUNK_SIZE;
/// Stable world-space partition used by generated biomes and simulation tuning.
/// This deliberately remains independent from the storage chunk size.
pub const WORLD_REGION_SIZE: u32 = 64;

pub const fn chunks_for_tile_radius(tile_radius: u32) -> u32 {
    tile_radius.div_ceil(CHUNK_SIZE as u32)
}
...
pub struct World {
    ...
    surveyed_areas: Vec<SurveyedArea>,
    survey_revision: u64,
...
impl World {
    pub fn empty(width: u32, height: u32, seed: u64) -> Result<Self, WorldError> {
        ...
        Ok(Self {
            ...
            chunks_high,
            chunks,
            biomes: BiomeMap::normal(
                width.div_ceil(WORLD_REGION_SIZE),
                height.div_ceil(WORLD_REGION_SIZE),
            ),
    ...
    pub fn surveyed_areas(&self) -> impl ExactSizeIterator<Item = SurveyedArea> + '_ {
        self.surveyed_areas.iter().copied()
    }

    pub const fn survey_revision(&self) -> u64 {
        self.survey_revision
    }

    pub fn is_surveyed(&self, position: TilePos) -> bool {
        self.surveyed_areas
            .iter()
            .any(|area| area.contains(position))
    }

    pub fn surveyed_area_intersects(&self, minimum: TilePos, maximum_exclusive: TilePos) -> bool {
        self.surveyed_areas
            .iter()
            .any(|area| area.intersects(minimum, maximum_exclusive))
    }
    ...
    /// Creates an immutable save snapshot while cloning the dense chunk planes
    /// across the available CPU cores. Runtime metadata remains a small serial
    /// clone; terrain is the dominant cost for large worlds.
    pub fn save_snapshot(&self) -> Result<Self, WorldError> {
        self.save_snapshot_with_threads(available_threads())
    }

    pub fn save_snapshot_with_threads(&self, threads: usize) -> Result<Self, WorldError> {
        let chunks = parallel_clone(&self.chunks, threads)?;
        Ok(Self {
            name: self.name.clone(),
            width: self.width,
            height: self.height,
            seed: self.seed,
            chunks_wide: self.chunks_wide,
            chunks_high: self.chunks_high,
            chunks,
            biomes: self.biomes.clone(),
            objects: self.objects.clone(),
            simulation_tick: self.simulation_tick,
            simulation_remainder_nanos: self.simulation_remainder_nanos,
            player_position_bits: self.player_position_bits,
            player_state: self.player_state.clone(),
            time_of_day_bits: self.time_of_day_bits,
            foreground_changes: self.foreground_changes.clone(),
            housing_changes: self.housing_changes,
            block_damage: self.block_damage.clone(),
            surveyed_areas: self.surveyed_areas.clone(),
            survey_revision: self.survey_revision,
...
fn parallel_clone<T>(values: &[T], threads: usize) -> Result<Vec<T>, WorldError>
where
    T: Clone + Send + Sync,
{
    if values.is_empty() {
    ...
    std::thread::scope(|scope| {
        let handles: Vec<_> = values
            .chunks(batch_size)
            .map(|batch| scope.spawn(move || batch.to_vec()))
            .collect();
        let mut cloned = Vec::with_capacity(values.len());
        for handle in handles {
            cloned.extend(handle.join().map_err(|_| WorldError::WorkerPanicked)?);
        }
        Ok(cloned)
    })
}
...
mod tests {
    ...
    fn tile_access_crosses_chunk_boundaries_and_keeps_layers_separate() {
        ...
        let mut world = World::empty(edge + 1, edge + 1, 9).unwrap();
        world
            .set_tile(edge, edge, Layer::Foreground, ForegroundTile::STONE)
            .unwrap();
        world
            .set_tile(edge, edge, Layer::Background, BackgroundTile::DIRT_WALL)
            .unwrap();
        assert_eq!(
            world.tile(edge, edge, Layer::Foreground).unwrap(),
            ForegroundTile::STONE
        );
        assert_eq!(
            world.tile(edge, edge, Layer::Background).unwrap(),
            BackgroundTile::DIRT_WALL
        );
        assert_eq!(world.chunk_count(), 4);
        assert!(world.tile(edge + 1, 0, Layer::Foreground).is_err());
    }
    ...
    #[test]
    fn parallel_save_snapshot_is_an_exact_independent_copy() {
        let mut world = World::empty(CHUNK_SIZE as u32 + 1, CHUNK_SIZE as u32 + 1, 7).unwrap();
        world
            .set_tile(
                CHUNK_SIZE as u32,
                CHUNK_SIZE as u32,
            ...
            .unwrap();
        let snapshot = world.save_snapshot_with_threads(4).unwrap();
        ...
        world
            ...
            .unwrap();
        assert_eq!(
            snapshot.tile(0, 0, Layer::Foreground).unwrap(),
            TileId::EMPTY
        );
    }

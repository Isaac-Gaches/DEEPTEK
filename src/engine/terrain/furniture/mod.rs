...
use crate::{ContractCompany, ItemId};
...
pub const LASER_BORE_WIDTH: u32 = 2;
...
/// Reusable simulation families. A new furniture variant selects one family in
/// its definition instead of teaching every simulation system another object ID.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum FurnitureBehavior {
    None,
    VerticalBore(VerticalBoreBehavior),
    DirectionalDrill,
    CargoLift,
    EnergyTurret,
    AmmunitionTurret,
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub struct VerticalBoreBehavior {
    pub shaft_width: u16,
    pub maximum_length: u32,
    pub first_scan_offset: u16,
    pub beam: BoreBeamVisual,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum BoreBeamVisual {
    Cyan,
    Red,
}
...
const fn shop_offer(
    ...
    price: u64,
    minimum_company_level: u8,
    description: &'static str,
) -> FurnitureOffer {
    FurnitureOffer {
        company,
        price,
        minimum_company_level,
        description,
    }
}
...
pub struct FurnitureDefinition {
    ...
    room_boundary: bool,
    noise_emission: u16,
    behavior: FurnitureBehavior,
    item: Option<FurnitureItem>,
    offer: Option<FurnitureOffer>,
}
...
impl FurnitureDefinition {
    pub const fn new(
        ...
        Self {
            ...
            room_boundary: false,
            noise_emission: 0,
            behavior: FurnitureBehavior::None,
            item: None,
            offer: None,
        }
    }
    ...
    pub const fn with_behavior(mut self, behavior: FurnitureBehavior) -> Self {
        self.behavior = behavior;
        self
    }

    pub const fn with_item(mut self, item: FurnitureItem) -> Self {
        self.item = Some(item);
        self
    }

    /// Adds the item to the procurement catalogue. The item, placement action,
    /// price and unlock requirements are all derived from this one definition.
    pub const fn with_offer(mut self, offer: FurnitureOffer) -> Self {
    ...
    pub const fn behavior(self) -> FurnitureBehavior {
        self.behavior
    }

    pub const fn item(self) -> Option<FurnitureItem> {
        self.item
    }

    pub const fn offer(self) -> Option<FurnitureOffer> {
        self.offer
    }
}
...
.with_item(catalogue_item(ItemId::CHEST, "Wooden Chest", 99, 6, 120))
...
.with_behavior(FurnitureBehavior::VerticalBore(VerticalBoreBehavior {
    shaft_width: LASER_BORE_WIDTH as u16,
    maximum_length: LASER_BORE_MAX_LENGTH,
    first_scan_offset: 1,
    beam: BoreBeamVisual::Cyan,
}))
.with_item(catalogue_item(ItemId::LASER_BORE, "Laser Bore", 99, 7, 750))
.with_offer(shop_offer(
    ContractCompany::DeepTekIndustries,
    1_500,
    0,
    "A powered two-column drill with local output storage for mined blocks.",
))
.with_chunk_activity()
...
.with_behavior(FurnitureBehavior::EnergyTurret)
.with_item(catalogue_item(ItemId::TURRET, "Defence Turret", 99, 8, 900))
.with_offer(shop_offer(
    ContractCompany::VanguardDefence,
    1_800,
    1,
    "An active powered defence platform with configurable target priority.",
))
.with_chunk_activity()
.with_facing()
...
.with_behavior(FurnitureBehavior::AmmunitionTurret)
.with_item(catalogue_item(
    ItemId::AMMO_TURRET,
    "Autocannon Turret",
    99,
    8,
...
.with_offer(shop_offer(
    ...
    2_100,
    2,
    "A rapid powered autocannon fed by crafted ballistic rounds or conveyors.",
))
.with_chunk_activity()
.with_facing()
.with_power(PowerRole::Consumer, [1, 0])
...
.with_behavior(FurnitureBehavior::DirectionalSentry)
.with_item(catalogue_item(
...
.with_offer(shop_offer(
    ...
    1_100,
    1,
    "A compact powered sentry block that fires straight along its facing lane.",
))
.with_chunk_activity()
.with_facing()
...
.with_item(catalogue_item(ItemId::SPIKES, "Spikes", 999, 0, 8));
...
.with_offer(shop_offer(
    ...
    3_000,
    1,
    "Exports stored cargo for money when powered and given a clear view of the sky.",
))
.with_chunk_activity()
.with_power(PowerRole::Consumer, [2, 0])
...
.with_offer(shop_offer(
    ...
    2_400,
    0,
    "Generates power during daylight and supplies machinery through pylons.",
))
.with_chunk_activity()
...
.with_item(catalogue_item(ItemId::PYLON, "Pylon", 999, 12, 100))
.with_local_chunk_activity()
...
.with_item(catalogue_item(ItemId::BATTERY, "Battery", 99, 13, 1_000))
.with_offer(shop_offer(
    ContractCompany::VanguardDefence,
    2_000,
    1,
    "Stores excess grid energy and supplies connected machinery after dark.",
))
.with_chunk_activity()
...
.with_behavior(FurnitureBehavior::CargoLift)
.with_item(catalogue_item(
    ItemId::CARGO_LIFT,
    "Cargo Lift",
    10,
...
.with_offer(shop_offer(
    ...
    2_400,
    2,
    "Powered moving cargo storage for vertical transport between lift stations.",
))
.with_chunk_activity()
.with_power(PowerRole::Consumer, [0, 0])
...
.with_offer(shop_offer(
    ...
    1_300,
    1,
    "Loads, unloads, and dispatches a cargo lift at a configured stop.",
))
.with_chunk_activity();
...
.with_offer(shop_offer(
    ...
    2_200,
    2,
    "Refines iron ore and combines dirt with stone when supplied with power.",
))
.with_chunk_activity()
.with_power(PowerRole::Consumer, [2, 0])
...
.with_behavior(FurnitureBehavior::VerticalBore(VerticalBoreBehavior {
    shaft_width: RED_SHAFT_BORE_WIDTH as u16,
    maximum_length: LASER_BORE_MAX_LENGTH,
    first_scan_offset: 0,
    beam: BoreBeamVisual::Red,
}))
.with_item(catalogue_item(
...
.with_offer(shop_offer(
    ...
    4_800,
    3,
    "A high-demand industrial bore that cuts a continuous four-tile shaft.",
))
.with_chunk_activity()
...
.with_behavior(FurnitureBehavior::DirectionalDrill)
.with_item(catalogue_item(
    ItemId::LASER_DRILL,
    "Laser Drill",
    99,
...
.with_offer(shop_offer(
    ...
    3_200,
    2,
    "A powered directional drill with seven aim presets and local output storage.",
))
.with_chunk_activity()
...
.with_item(catalogue_item(ItemId::DOOR, "Settlement Door", 99, 6, 60))
.with_offer(shop_offer(
    ContractCompany::DeepTekIndustries,
    120,
    0,
    "A three-tile settlement door that closes a specialist house boundary.",
))
.with_room_boundary();
...
.with_item(catalogue_item(ItemId::BED, "Settlement Bed", 99, 13, 80))
.with_offer(shop_offer(
    ContractCompany::DeepTekIndustries,
    150,
    0,
    "A settlement bed required for specialist housing and safe overnight rest.",
));
...
.with_offer(shop_offer(
    ...
    2_000,
    0,
    "Scans a wide strip to 700 m below its base and estimates every ore yield.",
))
.with_chunk_activity()
.with_power(PowerRole::Consumer, [2, 0])

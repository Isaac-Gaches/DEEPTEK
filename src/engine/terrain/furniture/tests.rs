...
#[test]
fn catalogue_items_and_shop_offers_are_derived_from_furniture_definitions() {
    let registry = crate::ItemRegistry::with_built_ins();
    for definition in BUILT_IN_FURNITURE
        .iter()
        .filter(|definition| definition.offer().is_some())
    {
        let item = definition.item().expect("shop furniture declares its item");
        let registered = registry.get(item.id).expect("catalogue item is registered");
        assert_eq!(registered.name, item.name);
        assert_eq!(registered.max_stack, item.max_stack);
        assert_eq!(registered.icon, item.icon);
        assert_eq!(registered.export_value, item.export_value);
        assert_eq!(
            registry.item_for_furniture(definition.object_type()),
            Some(item.id)
        );
        let listed = crate::machine_offer(item.id).expect("shop offer is generated");
        let offer = definition.offer().unwrap();
        assert_eq!(listed.price, offer.price);
        assert_eq!(listed.company, offer.company);
        assert_eq!(listed.minimum_company_level, offer.minimum_company_level);
        assert_eq!(listed.description, offer.description);
    }
}

#[test]
fn reusable_machine_families_are_definition_owned() {
    assert!(matches!(
        LASER_BORE_DEFINITION.behavior(),
        FurnitureBehavior::VerticalBore(VerticalBoreBehavior {
            shaft_width: 2,
    ...
    assert!(matches!(
        RED_SHAFT_BORE_DEFINITION.behavior(),
        FurnitureBehavior::VerticalBore(VerticalBoreBehavior {
            shaft_width: 4,
    ...
    assert_eq!(
        CARGO_LIFT_DEFINITION.behavior(),
        FurnitureBehavior::CargoLift
    );
    assert_eq!(
        TURRET_DEFINITION.behavior(),
        FurnitureBehavior::EnergyTurret
    );
    assert_eq!(
        AMMO_TURRET_DEFINITION.behavior(),

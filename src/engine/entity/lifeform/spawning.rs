...
use crate::entity::physics::{can_auto_jump, update_collider};
use crate::{
    BUILT_IN_FURNITURE, BrokenTile, ChunkActivity, ChunkPos, Collider, ItemStack, Layer, ObjectId,
...
pub struct LifeformSimulationUpdate {
    ...
    /// Cargo spilled from machines on the frame a lifeform disables them.
    pub machine_contents_dropped: Vec<(TilePos, ItemStack)>,
...
struct MachineTargetCandidates {
    standard: Vec<MachineTarget>,
    glowgnat: Vec<MachineTarget>,
}
        ...
        Self {
            ...
            maximum: ChunkPos {
                x: centre
                    ...
                    .min(simulation_regions_wide(world) - 1),
                y: centre
                    .y
                    .saturating_add(radius[1])
                    .min(simulation_regions_high(world) - 1),
            },
        }
    }
...
pub struct LifeformSimulation {
    ...
    machine_target_objects: Vec<ObjectId>,
    machine_target_candidates: HashMap<ChunkPos, MachineTargetCandidates>,
}
...
impl LifeformSimulation {
    pub fn new(config: LifeformSimulationConfig) -> Self {
        Self {
            ...
            machine_target_objects: Vec::new(),
            machine_target_candidates: HashMap::new(),
        }
    }
    ...
    fn refresh_machinery_chunks(&mut self, terrain: &World, player_chunks: ChunkRect) {
        ...
        self.machine_target_objects.clear();
        self.machine_target_candidates.clear();
        for definition in BUILT_IN_FURNITURE.iter().copied() {
            ...
            for object in terrain.objects_of_type(definition.object_type()) {
                ...
                if definition.maximum_health().is_some()
                    ...
                    && object.health() > 0
                {
                    let active_index = self.machine_target_objects.len();
                    self.machine_target_objects.push(object.id());
                    let position = [
                    ...
                    self.machine_targets
                        .entry(
                            position_chunk(terrain, position)
                                .unwrap_or_else(|| simulation_region(anchor)),
                        )
                        .or_default()
                        .push(MachineTarget {
                            object: object.id(),
                ...
                let first_chunk = simulation_region(anchor);
                let last_chunk = simulation_region(last);
                ...
                let maximum = ChunkPos {
                    x: last_chunk
                        .x
                        .saturating_add(radius[0])
                        .min(simulation_regions_wide(terrain) - 1),
                    y: last_chunk
                        .y
                        .saturating_add(radius[1])
                        .min(simulation_regions_high(terrain) - 1),
                };
        ...
        self.rebuild_machine_target_candidates(terrain);
    ...
    fn rebuild_machine_target_candidates(&mut self, terrain: &World) {
        let mut origin_chunks = Vec::with_capacity(self.machine_targets.len() * 9);
        for &target_chunk in self.machine_targets.keys() {
            let minimum_x = target_chunk
                .x
                .saturating_sub(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS);
            let maximum_x = target_chunk
                .x
                .saturating_add(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS)
                .min(simulation_regions_wide(terrain) - 1);
            let minimum_y = target_chunk
                .y
                .saturating_sub(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS);
            let maximum_y = target_chunk
                .y
                .saturating_add(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS)
                .min(simulation_regions_high(terrain) - 1);
            for y in minimum_y..=maximum_y {
                for x in minimum_x..=maximum_x {
                    origin_chunks.push(ChunkPos { x, y });
                }
            }
        }
        origin_chunks.sort_unstable_by_key(|chunk| (chunk.y, chunk.x));
        origin_chunks.dedup();

        let standard_threshold = self.config.minimum_hostile_attention;
        let glowgnat_threshold = standard_threshold.max(GLOWGNAT_MIN_MACHINERY_ATTENTION);
        for origin in origin_chunks {
            let minimum_x = origin.x.saturating_sub(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS);
            let maximum_x = origin
                .x
                .saturating_add(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS)
                .min(simulation_regions_wide(terrain) - 1);
            let minimum_y = origin.y.saturating_sub(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS);
            let maximum_y = origin
                .y
                .saturating_add(MACHINE_TARGET_SEARCH_RADIUS_CHUNKS)
                .min(simulation_regions_high(terrain) - 1);
            let mut candidates = MachineTargetCandidates::default();
            for y in minimum_y..=maximum_y {
                for x in minimum_x..=maximum_x {
                    ...
                    if attention >= standard_threshold {
                        candidates.standard.extend_from_slice(targets);
                    }
                    if attention >= glowgnat_threshold {
                        candidates.glowgnat.extend_from_slice(targets);
                    }
                }
            }
            if !candidates.standard.is_empty() || !candidates.glowgnat.is_empty() {
                self.machine_target_candidates.insert(origin, candidates);
            }
        }
    }
    ...
    fn update_active_lifeforms(
        ...
        let active_machine_targets = self
            .machine_target_objects
            .iter()
            .map(|&object| {
                terrain
                    .object(object)
                    .is_some_and(|object| object.is_active() && object.health() > 0)
            })
            .collect::<Vec<_>>();
        {
            ...
            for (_, (lifeform, transform, collider)) in query.iter() {
                ...
                let machine_target = self.nearest_machine_target(
                    system,
                    terrain,
                    lifeform.id,
                    transform.position,
                    &active_machine_targets,
                );
        ...
        for (machine, damage) in attacks {
            ...
            if result.disabled {
                ...
                if let Some(contents) = terrain.take_container_contents(machine) {
                    update
                        .machine_contents_dropped
                        .extend(contents.into_iter().map(|stack| (position, stack)));
                }
            }
        }
    ...
    fn nearest_machine_target(
        ...
        lifeform: LifeformId,
        position: [f32; 2],
        active_machine_targets: &[bool],
    ) -> Option<MachineTarget> {
        let chunk = position_chunk(terrain, position)?;
        let candidates = self.machine_target_candidates.get(&chunk)?;
        let requires_dense_machinery = system.definition(lifeform).is_some_and(|definition| {
            definition.minimum_spawn_attention >= GLOWGNAT_MIN_MACHINERY_ATTENTION
        });
        let candidates = if requires_dense_machinery {
            &candidates.glowgnat
        } else {
            &candidates.standard
        };
        ...
        for &target in candidates {
            if !active_machine_targets
                .get(target.active_index)
                .copied()
                .unwrap_or(false)
            {
                continue;
...
fn advance_lifeform(
    ...
    while remaining > f32::EPSILON {
        ...
        if collider.on_ground
            && collider.velocity[0].abs() > f32::EPSILON
            && lifeform.jump_cooldown_remaining <= 0.0
            && system.definition(lifeform.id).is_some_and(|definition| {
                !definition.locomotion.is_flying()
                    && definition.jump_speed > 0.0
                    && can_auto_jump(transform, collider, terrain, collider.velocity[0], 1)
            })
            && let Some(definition) = system.definition(lifeform.id)
        {
...
fn spawn_position_in_chunk(
    ...
    let chunk_size = crate::WORLD_REGION_SIZE;
...
fn position_chunk(terrain: &World, position: [f32; 2]) -> Option<ChunkPos> {
    ...
    Some(ChunkPos {
        x: position[0] as u32 / crate::WORLD_REGION_SIZE,
        y: position[1] as u32 / crate::WORLD_REGION_SIZE,
    })
}

fn simulation_region(position: crate::TilePos) -> ChunkPos {
    ChunkPos {
        x: position.x / crate::WORLD_REGION_SIZE,
        y: position.y / crate::WORLD_REGION_SIZE,
    }
}

fn simulation_regions_wide(terrain: &World) -> u32 {
    terrain.width().div_ceil(crate::WORLD_REGION_SIZE)
}

fn simulation_regions_high(terrain: &World) -> u32 {
    terrain.height().div_ceil(crate::WORLD_REGION_SIZE)
}

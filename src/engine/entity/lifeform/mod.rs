#[path = "../../../game/content/lifeforms.rs"]
mod built_ins;
mod spawning;

pub use built_ins::BUILT_IN_LIFEFORMS;
pub use spawning::{
...
impl LifeformMaterials {
    ...
    const fn for_visual(self, visual: LifeformVisual) -> Handle<Material> {
        match visual {
            LifeformVisual::Walker => self.walker,
            LifeformVisual::Glowgnat => self.glowgnat,
        }
    }
}
...
pub struct LifeformDefinition {
    ...
    /// Inclusive seconds between optional ambient jumps. `[0, 0]` disables them.
    pub random_jump_interval: [f32; 2],
    ...
    pub spawn_biomes: &'static [BiomeId],
    pub minimum_spawn_attention: u32,
    pub spawn_weight: u16,
}

impl LifeformDefinition {
    pub const fn walker(id: LifeformId, name: &'static str) -> Self {
        Self {
            id,
            name,
            visual: LifeformVisual::Walker,
            emissive: 0.0,
            maximum_health: 40,
            ...
            spawn_biomes: &[BiomeId::NORMAL, BiomeId::GLOWING_CRYSTAL],
            minimum_spawn_attention: 0,
            spawn_weight: 100,
        }
    }

    pub const fn glowgnat(id: LifeformId, name: &'static str) -> Self {
        Self {
            id,
            name,
            visual: LifeformVisual::Glowgnat,
            emissive: 0.85,
            maximum_health: 18,
            ...
            block_attack_damage: 4,
            block_attack_interval: 5.0 / 3.0,
            spawn_biomes: &[BiomeId::GLOWING_CRYSTAL],
    ...
    fn is_valid(&self) -> bool {
        self.maximum_health > 0
            ...
            && (0.0..=1.0).contains(&self.emissive)
            ...
            && self.random_jump_interval[0] >= 0.0
            && self.random_jump_interval[1] >= self.random_jump_interval[0]
...
impl Lifeform {
    ...
    fn schedule_random_jump(&mut self, interval: [f32; 2]) {
        if interval[1] <= 0.0 {
            self.random_jump_remaining = f32::INFINITY;
            return;
        }
        self.random_jump_remaining = interval[0] + self.next_random() * (interval[1] - interval[0]);
    }
...
impl LifeformSystem {
    ...
    pub fn with_built_ins() -> Self {
        ...
        for &definition in BUILT_IN_LIFEFORMS {
            system
                .register(definition)
    ...
    pub fn spawn(
        ...
        let sprite = Sprite::new(materials.for_visual(definition.visual))
            .with_tint(definition.tint)
            .with_emissive(definition.emissive);
        Ok(entities.spawn((
...
fn update_grounded_lifeform(
    ...
    if lifeform.random_jump_remaining < 0.0 {
        lifeform.schedule_random_jump(definition.random_jump_interval);
    } else {
        lifeform.random_jump_remaining = (lifeform.random_jump_remaining - dt).max(0.0);
    }
    let stuck_jump = lifeform.stuck_for >= definition.stuck_duration;
    let random_jump = lifeform.random_jump_remaining <= 0.0;
    if collider.on_ground
        && definition.jump_speed > 0.0
        && lifeform.jump_cooldown_remaining <= 0.0
        && (stuck_jump || random_jump)
    {
...
/// Compatibility helper for callers that need an owned definition collection.
pub fn built_in_lifeform_definitions() -> [LifeformDefinition; 2] {
    [BUILT_IN_LIFEFORMS[0], BUILT_IN_LIFEFORMS[1]]
}
...
mod tests {
    ...
    fn definitions_are_data_driven_and_reject_duplicates() {
        let built_ins = LifeformSystem::with_built_ins();
        assert_eq!(built_ins.definitions().count(), BUILT_IN_LIFEFORMS.len());
        assert!(BUILT_IN_LIFEFORMS.iter().all(LifeformDefinition::is_valid));
    ...
    #[test]
    fn grounded_walker_jumps_instead_of_entering_a_recovery_cycle_when_stuck() {
        let system = LifeformSystem::with_built_ins();
        let mut entities = World::new();
        let target = entities.spawn((Transform::new([10.0, 0.0]),));
        let walker = entities.spawn((
        ...
        assert_eq!(
            collider.velocity[1],
            -system.definition(LifeformId::WALKER).unwrap().jump_speed
        );
        assert!(!collider.on_ground);
        drop(collider);
        let lifeform = entities.get::<&Lifeform>(walker).unwrap();
        assert!(!lifeform.is_recovering());
    }

    #[test]
    fn grounded_walker_jumps_at_bounded_random_intervals() {
        let system = LifeformSystem::with_built_ins();
        let definition = system.definition(LifeformId::WALKER).unwrap();
        let mut lifeform = Lifeform::new(LifeformId::WALKER, [0.0, 0.0]);
        ...
        assert!(jumped);
        assert!(lifeform.random_jump_remaining >= definition.random_jump_interval[0]);
        assert!(lifeform.random_jump_remaining <= definition.random_jump_interval[1]);
    }

...
/// Returns whether a grounded collider can jump onto a single blocking tile in
/// its direction of travel. The lifted probe covers the complete collider, so
/// low ceilings and taller walls do not trigger an automatic jump.
pub(super) fn can_auto_jump(
    transform: &Transform,
    collider: &Collider,
    terrain: &World,
    direction: f32,
    ...
    if !collider.on_ground || direction.abs() <= f32::EPSILON {
        return false;
    }
    ...
    let leading_edge = centre[0] + collider.half_extents[0] * direction;
    let first_ahead_x = if direction > 0.0 {
        (leading_edge + 0.5 + COLLISION_EPSILON).floor() as i32
    } else {
        (leading_edge - 0.5 - COLLISION_EPSILON).ceil() as i32
    };
    let feet_y = (centre[1] + collider.half_extents[1] + 0.5 + COLLISION_EPSILON).floor() as i32;
    let obstacle_y = feet_y - 1;
    for offset in 0..lookahead_tiles.max(1) {
        let ahead_x = first_ahead_x + direction as i32 * offset as i32;
        if !tile_is_solid(terrain, ahead_x, obstacle_y) {
            continue;
        }
        if tile_is_solid(terrain, ahead_x, obstacle_y - 1) {
            return false;
        }

        let mut lifted = *transform;
        lifted.position[0] += direction * (offset as f32 + 0.25);
        lifted.position[1] -= 1.0 + COLLISION_EPSILON;
        return first_overlapping_tile(&lifted, collider, terrain, 0, direction).is_none();
    }
    false
}
...
mod tests {
    ...
    #[test]
    fn auto_jump_requires_a_single_step_with_clear_headroom() {
        let mut terrain = floor_world();
        terrain
            .set_tile(5, 5, Layer::Foreground, ForegroundTile::STONE)
            .unwrap();
        let transform = Transform::new([4.0, 4.4999]);
        let collider = Collider {
            on_ground: true,
            ..Collider::new(1.0, 2.0)
        };

        assert!(can_auto_jump(&transform, &collider, &terrain, 1.0, 1));
        assert!(!can_auto_jump(&transform, &collider, &terrain, -1.0, 1));

        terrain
            .set_tile(5, 4, Layer::Foreground, ForegroundTile::STONE)
            .unwrap();
        assert!(!can_auto_jump(&transform, &collider, &terrain, 1.0, 1));
    }

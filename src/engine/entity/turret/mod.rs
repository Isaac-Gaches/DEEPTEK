...
#[cfg(test)]
use crate::FurnitureObject;
use crate::{
    CHUNK_SIZE, ChunkPos, FurnitureBehavior, FurnitureFacing, ItemId, Layer, ObjectId, PowerSystem,
    SpecialistBonuses, TargetPriority, TileId, World as TerrainWorld, furniture_definition,
};
...
impl TurretKind {
    fn from_object_type(object_type: crate::ObjectTypeId) -> Option<Self> {
        match furniture_definition(object_type)?.behavior() {
            FurnitureBehavior::EnergyTurret => Some(Self::Energy),
            FurnitureBehavior::AmmunitionTurret => Some(Self::Ammunition),
            FurnitureBehavior::DirectionalSentry => Some(Self::Directional),
            _ => None,
        }
    }

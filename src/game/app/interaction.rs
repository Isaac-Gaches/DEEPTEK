...
const CONTINUOUS_ITEM_USE_INTERVAL: Duration = Duration::from_millis(85);
...
pub(crate) fn handle_pointer_actions(
    ...
    if std::mem::take(&mut input.interaction_queued) {
        ...
        if let Some(specialist) = specialist {
            ...
            input.clear_focus();
            interacted = true;
        } else if player_can_reach(entities, player, world_position)
            && let Some(target) = world_tile_at(world_position, world)
        {
...
fn specialist_at(
    ...
    if !player_can_reach(entities, player, world_position) {
        return None;
    }
    entities
...
fn player_can_reach(
    ...
    player
        .and_then(|player| entity_position(entities, player))
        .is_some_and(|position| {
            distance_squared(position, world_position) <= DEFAULT_ITEM_REACH.powi(2)
        })
}
...
mod tests {
    ...
    #[test]
    fn world_interaction_reach_is_five_tiles_inclusive() {
        let mut entities = EntityWorld::new();
        let player = entities.spawn((Transform::new([10.0, 10.0]),));

        assert!(player_can_reach(&entities, Some(player), [13.0, 14.0]));
        assert!(!player_can_reach(&entities, Some(player), [13.1, 14.0]));
        assert!(!player_can_reach(&entities, None, [10.0, 10.0]));
    }
}

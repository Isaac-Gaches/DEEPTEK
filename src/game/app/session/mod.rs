...
struct App {
    ...
    contracts_attention: bool,
    known_active_contracts: usize,
    known_claimable_contracts: usize,
...
impl App {
    fn new() -> Self {
        ...
        Self {
            ...
            contracts_attention: false,
            known_active_contracts: 0,
            known_claimable_contracts: 0,
    ...
    fn update_entities(&mut self, elapsed: f32, time_of_day: f32) -> [f32; 2] {
            ...
            && let Some(materials) = self.lifeform_materials
        {
            ...
            if let Some(material) = self.dropped_item_material {
                for (position, stack) in lifeform_update.machine_contents_dropped {
                    self.dropped_item_system.spawn_stack(
                        &mut self.entities,
                        material,
                        &self.item_registry,
                        stack,
    ...
    fn start_world(&mut self, world: World, path: PathBuf, is_new_world: bool) {
        ...
        self.contracts_attention = false;
        self.known_active_contracts = self.contract_board.active().len();
        self.known_claimable_contracts = self
            .contract_board
            .active()
            .iter()
            .filter(|contract| contract.is_complete())
            .count();
    ...
    fn poll_autosave(&mut self, now: Instant) {
            ...
            && let Some(path) = self.active_world_path.clone()
        {
            ...
            let snapshot = match self.world.save_snapshot() {
                Ok(snapshot) => snapshot,
                Err(error) => {
                    eprintln!("autosave snapshot failed: {error}");
                    self.save_notice = Some(SaveNotice {
                        message: "SAVE FAILED",
                        is_error: true,
                        expires_at: now + SAVE_NOTICE_DURATION,
            ...
            self.save_job = Some(std::thread::spawn(move || snapshot.save(path)));
            self.last_autosave = now;
        }
    }
    ...
    fn handle_pause_menu_action(&mut self, action: PauseMenuAction) {
        match action {
            ...
            PauseMenuAction::SetRenderDistance(distance) => {
                ...
                if let (Some(gpu), Some(terrain), Some(sprites)) = (
                    &mut self.gpu,
                    &mut self.terrain_renderer,
                    &self.sprite_renderer,
                ) && terrain.set_render_distance(gpu, horizontal, vertical)
                {
                    sprites.rebind_lighting(gpu, terrain);
                    terrain.release_retired_lighting_textures(gpu);
                }
        ...
        match self.screen {
            AppScreen::Playing => {
                ...
                if let Some(position) = self
                    ...
                    .and_then(|player| entity_position(&self.entities, player))
                {
                    self.world_map_gui.focus_on_player(position);
                }
    ...
    fn hud_snapshot(&self) -> HudSnapshot {
        ...
        HudSnapshot {
            ...
            contracts_attention: self.contracts_attention,
        }
    }

    fn refresh_contract_attention(&mut self) {
        let active = self.contract_board.active().len();
        let claimable = self
            .contract_board
            .active()
            .iter()
            .filter(|contract| contract.is_complete())
            .count();
        if active > self.known_active_contracts || claimable > self.known_claimable_contracts {
            self.contracts_attention = true;
        }
        self.known_active_contracts = active;
        self.known_claimable_contracts = claimable;
    }

    fn mark_contracts_viewed(&mut self) {
        self.contracts_attention = false;
        self.known_active_contracts = self.contract_board.active().len();
        self.known_claimable_contracts = self
            .contract_board
                ...
                let result = self
                    ...
                    .map_or(Err(PurchaseError::ItemNotOffered), |mut wallet| {
                        self.delivery_system
                            .purchase(item, &mut wallet, self.corporation_progress)
                    });
                if result.is_ok() {
...
fn demo_terrain_config(render_distance: RenderDistance) -> TerrainRenderConfig {
    let (horizontal_chunk_radius, vertical_chunk_radius) = render_distance.chunk_radii();
    TerrainRenderConfig {
        horizontal_chunk_radius,
        vertical_chunk_radius,

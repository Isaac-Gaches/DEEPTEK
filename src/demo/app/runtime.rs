...
impl ApplicationHandler for App {
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let mut terrain_renderer = TerrainRenderer::new(&mut gpu, demo_terrain_config());
        let (horizontal, vertical) = self.render_distance.chunk_radii();
        terrain_renderer.set_render_distance(horizontal, vertical);
        ...
        let lifeform_texture = gpu
            .load_texture_from_file(include_bytes!("../../../assets/entities/player.png").to_vec());
            ...
            WindowEvent::MouseInput { state, button, .. } => match (button, state) {
                (MouseButton::Left, ElementState::Pressed) => {
                    ...
                    if self.procurement_gui.is_open() {
                        if let Some(action) = self.procurement_gui.handle_click(
                            self.input.cursor_position(),
                            viewport,
                            &self.contract_board,
                        ) {
                    ...
                    if self.screen == AppScreen::WorldMenu {
                        ...
                        if self.procurement_gui.is_open() {
                            self.procurement_gui
                                .scroll(direction, viewport, &self.contract_board);
                        } else if self.inventory_gui.scroll_crafting(direction) {

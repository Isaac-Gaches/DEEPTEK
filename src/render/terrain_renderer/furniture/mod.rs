...
use crate::{
    BoreBeamVisual, ChunkPos, FurnitureBehavior, FurnitureObject, FurnitureSupport,
    ItemTransportShape, ObjectId, PowerConnection, PowerSystem, TilePos, World, WorldObject,
    furniture_definition, item_transport_shape, render_common,
};
use easy_gpu::assets::{
...
#[cfg(test)]
const LASER_BORE_BEAM_WIDTH: f32 = 2.0;
...
impl LaserParticleEmitter {
    ...
    const fn cyan_wide(impact: Option<[f32; 2]>, width: f32) -> Self {
        Self {
            impact,
            width,
            kind: LaserParticleKind::Cyan,
...
impl FurnitureRenderer {
    ...
    pub(super) fn rebind_lighting(&self, gpu: &mut easy_gpu::Renderer, lighting: &LightingEngine) {
        render_common::replace_material(
            gpu,
            self.material,
            MaterialBuilder::new(self.material_pipeline)
        ...
        render_common::replace_material(
            gpu,
            self.cable_material,
            MaterialBuilder::new(self.cable_pipeline)
                .buffer(0, self.camera_buffer)
                .texture(1, lighting.light_texture)
                .sampler(2, lighting.light_sampler)
                .buffer(3, lighting.light_meta_buffer),
...
fn append_laser_bore(
    ...
    if let Some(beam) = world.laser_bore_beam(object, power.is_powered(object.id())) {
        ...
        particle_emitters.push(LaserParticleEmitter::cyan_wide(
...
fn append_furniture_instance(
    ...
    if matches!(
        ...
        output.push(FurnitureInstance {
            position: [
                ...
                -(anchor.y as f32 + (f32::from(height) - 1.0) * 0.5 + FURNITURE_GROUND_INSET),
                0.25,
            ],
    ...
    if definition.behavior() == FurnitureBehavior::DirectionalDrill {
    ...
    if definition.behavior() == FurnitureBehavior::AmmunitionTurret {
    ...
    if definition.behavior() == FurnitureBehavior::DirectionalSentry {
        let facing = world.furniture_facing(object.id()).unwrap_or_default();
        output.push(FurnitureInstance {
            position: [anchor.x as f32, -(anchor.y as f32), 0.25],
            size: [1.0, 1.0],

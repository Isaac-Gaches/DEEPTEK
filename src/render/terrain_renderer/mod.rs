...
pub use lighting::{
    LightSource, LightingUpdateStats, lighting_tile_dimensions, prepare_lighting_window,
    update_lighting_window_cells,
};
pub use mesh_builder::{ChunkMeshData, TerrainVertex, build_chunk_mesh, build_chunk_mesh_into};

use crate::{
    CHUNK_SIZE, ChunkPos, Layer, PowerSystem, TileId, World, WorldError, chunks_for_tile_radius,
    render_common,
};
use easy_gpu::assets::{
    Buffer, BufferUsages, GpuVertex, Material, MaterialBuilder, Mesh, RenderPipeline,
...
impl Default for TerrainRenderConfig {
    fn default() -> Self {
        Self {
            horizontal_chunk_radius: chunks_for_tile_radius(320),
            vertical_chunk_radius: chunks_for_tile_radius(192),
            unload_margin: chunks_for_tile_radius(64),
...
pub struct TerrainRenderer {
    ...
    mesh_data_pool: Vec<ChunkMeshData>,
    region_job_scratch: Vec<(ChunkPos, Layer)>,
    region_mesh_scratch: ChunkMeshData,
    materials: [Handle<Material>; 2],
    material_pipelines: [Handle<RenderPipeline>; 2],
    terrain_textures: [Handle<Texture>; 2],
    terrain_sampler: Handle<Sampler>,
    ...
    retired_lighting_textures: Vec<Handle<Texture>>,
    lighting_dirty: bool,
...
impl TerrainRenderer {
    pub fn new(gpu: &mut easy_gpu::Renderer, config: TerrainRenderConfig) -> Self {
        ...
        let background_pipeline = RenderPipelineBuilder::new(shader)
            ...
            .build(gpu);
        let foreground_texture = gpu.load_texture_from_file(
            include_bytes!("../../../assets/terrain/fg_tiles.png").to_vec(),
        );
        let background_texture = gpu.load_texture_from_file(
            include_bytes!("../../../assets/terrain/bg_tiles.png").to_vec(),
        );
        ...
        let mut renderer = Self {
            ...
            mesh_data_pool: Vec::new(),
            region_job_scratch: Vec::new(),
            region_mesh_scratch: ChunkMeshData::default(),
            materials: [background_material, foreground_material],
            material_pipelines: [background_pipeline, foreground_pipeline],
            terrain_textures: [background_texture, foreground_texture],
            terrain_sampler: texture_sampler,
            camera_buffer,
            decorations,
            furniture,
            lighting,
            retired_lighting_textures: Vec::new(),
            lighting_dirty: true,
    ...
    /// Changes both the streamed terrain window and the physical light-map size.
    /// Returns true when lighting-dependent external materials must be rebound.
    pub fn set_render_distance(
        ...
        self.active_horizontal_chunk_radius = horizontal_chunks;
        self.active_vertical_chunk_radius = vertical_chunks;
        let Some(retired) = self
            .lighting
            .resize(gpu, horizontal_chunks, vertical_chunks)
        else {
            return false;
        };
        self.retired_lighting_textures.extend(retired);
        self.rebind_lighting_materials(gpu);
        self.lighting_dirty = true;
        self.lighting_occupancy_dirty = true;
        true
    }

    /// Releases old light-map textures after all external lit materials have
    /// rebound to the current handles.
    pub fn release_retired_lighting_textures(&mut self, gpu: &mut easy_gpu::Renderer) {
        for texture in self.retired_lighting_textures.drain(..) {
            let _ = gpu.asset_manager.textures.remove(texture);
        }
    }

    pub const fn lighting_dimensions(&self) -> [u32; 2] {
        self.lighting.tile_dimensions()
    }

    fn rebind_lighting_materials(&mut self, gpu: &mut easy_gpu::Renderer) {
        render_common::replace_material(
            gpu,
            self.materials[layer_index(Layer::Foreground)],
            MaterialBuilder::new(self.material_pipelines[layer_index(Layer::Foreground)])
                .buffer(0, self.camera_buffer)
                .texture(1, self.terrain_textures[layer_index(Layer::Foreground)])
                .sampler(2, self.terrain_sampler)
                .texture(3, self.lighting.light_texture)
                .sampler(4, self.lighting.light_sampler)
                .buffer(5, self.lighting.light_meta_buffer),
        );
        render_common::replace_material(
            gpu,
            self.materials[layer_index(Layer::Background)],
            MaterialBuilder::new(self.material_pipelines[layer_index(Layer::Background)])
                .buffer(0, self.camera_buffer)
                .texture(1, self.terrain_textures[layer_index(Layer::Background)])
                .sampler(2, self.terrain_sampler)
                .texture(3, self.lighting.light_texture)
                .sampler(4, self.lighting.light_sampler)
                .buffer(5, self.lighting.light_meta_buffer)
                .texture(6, self.lighting.occlusion_texture),
        );
        self.decorations.rebind_lighting(gpu, &self.lighting);
        self.furniture.rebind_lighting(gpu, &self.lighting);
    }
    ...
    pub fn sync(
        ...
        while let Some(position) = self.despawn_scratch.pop() {
            if let Some(mut chunk) = self.chunks.remove(&position) {
                for mesh in &mut chunk.meshes {
                    mesh.vertices.clear();
                    mesh.indices.clear();
                    self.mesh_data_pool.push(std::mem::take(mesh));
                }
        ...
        let generated = build_jobs(
            &self.mesh_pool,
            world,
            &self.mesh_job_scratch,
            &mut self.mesh_data_pool,
        );
        for (position, layer, mesh_data) in generated {
            let index = layer_index(layer);
            if let Some(chunk) = self.chunks.get_mut(&position) {
                let mut previous = std::mem::replace(&mut chunk.meshes[index], mesh_data);
                previous.vertices.clear();
                previous.indices.clear();
                self.mesh_data_pool.push(previous);
    ...
    pub fn mark_tile_dirty(&mut self, x: u32, y: u32, layer: Layer) {
        self.lighting_dirty = true;
        self.lighting_occupancy_dirty = true;
        self.lighting.mark_tile_dirty(crate::TilePos::new(x, y));
...
fn build_jobs(
    ...
    world: &World,
    jobs: &[(ChunkPos, Layer)],
    mesh_data_pool: &mut Vec<ChunkMeshData>,
) -> Vec<(ChunkPos, Layer, ChunkMeshData)> {
    let jobs = jobs
        .iter()
        .map(|&(position, layer)| (position, layer, mesh_data_pool.pop().unwrap_or_default()))
        .collect::<Vec<_>>();
    pool.install(|| {
        jobs.into_par_iter()
            .map(|(position, layer, mut mesh)| {

...
impl HudGui {
    ...
    pub fn queue(
        ...
        if snapshot.contracts_attention {
            let badge = Rect {
                centre: [layout.contracts.right() - 3.0, layout.contracts.top() + 3.0],
                size: [22.0, 22.0],
            };
            renderer.queue_rect(badge.centre, badge.size, [0.95, 0.38, 0.08, 1.0]);
            renderer.queue_text(
                "!",
                [badge.centre[0] - 3.0, badge.centre[1] - 6.0],
                1.8,
                [1.0, 1.0, 0.9, 1.0],
            );
        }

use crate::{
    Collider, ContractCompany, Inventory, ItemId, ItemRegistry, Sprite, Transform, Wallet,
};
...
pub const DELIVERY_DELAY_SECONDS: f32 = 10.0;
...
pub const DELIVERY_DROP_HEIGHT: f32 = 10.0;
...
pub const MACHINE_OFFERS: &[MachineOffer] = &[
    MachineOffer {
        item: ItemId::LASER_BORE,
        company: ContractCompany::DeepTekIndustries,
        price: 1_500,
        description: "A powered single-column drill with local output storage for mined blocks.",
    },
    MachineOffer {
        item: ItemId::RED_SHAFT_BORE,
        company: ContractCompany::AstraSurveyCorp,
        price: 4_800,
        description: "A high-demand industrial bore that cuts a continuous four-tile shaft.",
    },
    MachineOffer {
        item: ItemId::COMPOSITE_ASSEMBLER,
        company: ContractCompany::DeepTekIndustries,
        price: 2_200,
        description: "Refines iron ore and combines dirt with stone when supplied with power.",
    },
    MachineOffer {
        item: ItemId::TURRET,
        company: ContractCompany::VanguardDefence,
        price: 1_800,
        description: "An active powered defence platform with configurable target priority.",
    },
    MachineOffer {
        item: ItemId::AMMO_TURRET,
        company: ContractCompany::VanguardDefence,
        price: 2_100,
        description: "A rapid powered autocannon fed by crafted ballistic rounds or conveyors.",
    },
    MachineOffer {
        item: ItemId::DIRECTIONAL_SENTRY,
        company: ContractCompany::VanguardDefence,
        price: 1_100,
        description: "A compact powered sentry block that fires straight along its facing lane.",
    },
    MachineOffer {
        item: ItemId::ORBITAL_EXPORT_LAUNCHER,
        company: ContractCompany::DeepTekIndustries,
        price: 3_000,
        description: "Exports stored cargo for money when powered and given a clear view of the sky.",
    },
    MachineOffer {
        item: ItemId::SOLAR_ARRAY,
        company: ContractCompany::AstraSurveyCorp,
        price: 2_400,
        description: "Generates power during daylight and supplies machinery through pylons.",
    },
    MachineOffer {
        item: ItemId::BATTERY,
        company: ContractCompany::VanguardDefence,
        price: 2_000,
        description: "Stores excess grid energy and supplies connected machinery after dark.",
    },
    MachineOffer {
        item: ItemId::CARGO_LIFT,
        company: ContractCompany::DeepTekIndustries,
        price: 2_400,
        description: "Powered moving cargo storage for vertical transport between lift stations.",
    },
    MachineOffer {
        item: ItemId::LIFT_STATION,
        company: ContractCompany::DeepTekIndustries,
        price: 1_300,
        description: "Loads, unloads, and dispatches a cargo lift at a configured stop.",
    },
    MachineOffer {
        item: ItemId::LASER_DRILL,
        company: ContractCompany::AstraSurveyCorp,
        price: 3_200,
        description: "A powered directional drill with seven aim presets and local output storage.",
    },
    MachineOffer {
        item: ItemId::SUBSURFACE_SURVEYOR,
        company: ContractCompany::AstraSurveyCorp,
        price: 2_000,
        description: "Scans a wide strip to 700 m below its base and estimates every ore yield.",
    },
    MachineOffer {
        item: ItemId::DOOR,
        company: ContractCompany::DeepTekIndustries,
        price: 120,
        description: "A three-tile settlement door that closes a specialist house boundary.",
    },
    MachineOffer {
        item: ItemId::BED,
        company: ContractCompany::DeepTekIndustries,
        price: 150,
        description: "A settlement bed required for specialist housing and safe overnight rest.",
    },
];
...
impl DeliverySystem {
    pub fn purchase(&mut self, item: ItemId, wallet: &mut Wallet) -> Result<(), PurchaseError> {
...
mod tests {
    ...
    #[test]
    fn purchase_charges_once_and_delivers_after_ten_seconds() {
        let mut system = DeliverySystem::default();
        let mut wallet = Wallet::new(2_000);
        let mut entities = EntityWorld::new();

        assert_eq!(system.purchase(ItemId::LASER_BORE, &mut wallet), Ok(()));
        assert_eq!(wallet.money(), 500);
        assert_eq!(system.pending_count(), 1);
        assert_eq!(
            system.update_queue(&mut entities, material(), 10.0, 5.0, 30, 9.9),
            0
        );
        assert_eq!(
    ...
    fn rejected_purchase_never_charges_or_enters_the_queue() {
        ...
        assert_eq!(
            system.purchase(ItemId::LASER_BORE, &mut wallet),
    ...
    fn program_batch_is_atomic_and_uses_the_standard_delivery_delay() {
        let mut system = DeliverySystem::default();
        let mut wallet = Wallet::new(2_000);
        system.purchase(ItemId::LASER_BORE, &mut wallet).unwrap();
        let items = [

...
impl InventoryGui {
    ...
    pub fn scroll_crafting(&mut self, direction: f32) -> bool {
        if !self.open || self.open_container.is_some() || self.page != InventoryPage::Crafting {
            return false;
        }
        let total_rows = CRAFTING_RECIPES.len().div_ceil(CRAFTING_COLUMNS);
    ...
    pub fn handle_click(
        ...
        if personal_inventory {
            ...
            if self.page == InventoryPage::Crafting
                && let Some(recipe) = CRAFTING_RECIPES
                    .iter()
                    .copied()
            ...
            if self.controls.subsurface_survey_status {
                ...
                let colour = if !active {
                    ...
                    for estimate in survey.estimates() {
                        ...
                        lines.push(format!("{name}: ~{} YIELD", estimate.estimated_yield));
                    }
                    if lines.len() == 1 {

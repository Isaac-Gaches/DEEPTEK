...
use crate::{
    BackgroundTile, ForegroundTile, FurnitureObject, ItemRegistry, Layer, TileId, World,
...
impl MapView {
    ...
    fn zoom_by(&mut self, factor: f32) {
        self.zoom = (self.zoom * factor).clamp(1.0, MAP_MAX_ZOOM);
        self.clamp_centre();
    }
...
impl MapLayout {
    fn new(viewport: [f32; 2], world_size: [u32; 2], zoomed: bool) -> Self {
        let available_width = (viewport[0] - 80.0).max(1.0);
        let available_height = (viewport[1] - 126.0).max(1.0);
        let size = if zoomed {
            [available_width, available_height]
        } else {
            let scale = (available_width / world_size[0].max(1) as f32)
                .min(available_height / world_size[1].max(1) as f32);
            [
                (world_size[0] as f32 * scale).max(1.0),
                (world_size[1] as f32 * scale).max(1.0),
            ]
        };
        Self {
...
impl WorldMapGui {
    ...
    pub fn upload_if_needed(
        ...
        let layout = MapLayout::new(viewport, self.world_size, self.view.zoom > 1.0);
    ...
    pub fn queue(
        ...
        let layout = MapLayout::new(viewport, world_size, self.view.zoom > 1.0);
        renderer.queue_rect(
            layout.centre,
        ...
        let sea_normalized = world.sea_level_y() as f32 / world.height().max(1) as f32;
        if (view_minimum[1]..=view_maximum[1]).contains(&sea_normalized) {
            let sea_level = layout.normalized_to_screen(
                [view_minimum[0], sea_normalized],
                world_size,
                self.view,
            )[1];
            renderer.queue_world_map_overlay(
                [layout.centre[0], sea_level],
                [layout.size[0], 1.0],
                [0.1, 0.75, 1.0, 0.58],
            );
        }
    ...
    fn synchronize_furniture(&mut self, world: &World, registry: &ItemRegistry) {
        ...
        self.furniture.extend(world.objects().filter_map(|object| {
            ...
            let x = if object.object_type() == FurnitureObject::CARGO_LIFT {
                object.anchor().x as f32 + 0.5
            } else {
                object.anchor().x as f32 + f32::from(size[0]) * 0.5
            };
            let y = if object.object_type() == FurnitureObject::CARGO_LIFT {
                object.motion_position_tiles() + 1.0
            } else {

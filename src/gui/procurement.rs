...
impl ProcurementGui {
        ...
        match self.tab {
            ProcurementTab::Catalogue => {
                for visible_index in 0..layout.visible_rows {
                    let offer_index = self.scroll_offset + visible_index;
                    if offer_index >= MACHINE_OFFERS.len() {
                        break;
                    }
                ...
                if layout.action.contains(cursor)
                    && let Some(offer) = MACHINE_OFFERS.get(self.selected)
                {
    ...
    pub fn scroll(&mut self, direction: f32, viewport: [f32; 2], contracts: &ContractBoard) {
        ...
        let (offset, item_count) = match self.tab {
            ProcurementTab::Catalogue => (&mut self.scroll_offset, MACHINE_OFFERS.len()),
            ProcurementTab::Contracts => (
    ...
    pub fn queue(
        ...
        for visible_index in 0..layout.visible_rows {
            ...
            let company = offer.company.short_name();
        ...
        if MACHINE_OFFERS.len() > layout.visible_rows {
            renderer.queue_text(
                &format!(
                    "{}-{} / {}   SCROLL",
                    self.scroll_offset + 1,
                    (self.scroll_offset + layout.visible_rows).min(MACHINE_OFFERS.len()),
                    MACHINE_OFFERS.len()
                ),
        ...
        if let Some(offer) = MACHINE_OFFERS.get(self.selected) {
            renderer.queue_rect(
            ...
            renderer.queue_text(
                ...
                [layout.details.left() + 18.0, layout.details.top() + 218.0],
                1.25,
                [0.68, 0.77, 0.84, 1.0],
            );
            queue_centred_text(
        ...
        if let Some(feedback) = self.feedback {
            let (text, colour) = match feedback {
                TerminalFeedback::Ordered => (
                    "ORDER ACCEPTED - DROP IN 10 SECONDS",
                    [0.40, 1.0, 0.58, 1.0],
                ),

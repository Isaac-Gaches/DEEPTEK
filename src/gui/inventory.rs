...
use crate::{
    ...
    Inventory, ItemAction, ItemCategory, ItemContainer, ItemRegistry, ItemStack, LaserDrillAim,
    Layer, LiftStationConfiguration, LiftStationMode, ObjectId, ProjectileKind, SlotClick,
    SubsurfaceSurvey, TargetPriority, ToolAction,
};
...
impl CraftingLayout {
    ...
    fn recipe_button(self, index: usize) -> ([f32; 2], [f32; 2]) {
        let gap = 4.0;
        let padding = 8.0;
        let top = self.panel.0[1] - self.panel.1[1] * 0.5 + padding + INVENTORY_TAB_HEIGHT + gap;
...
fn queue_crafting_panel(
    ...
    for (index, recipe) in CRAFTING_RECIPES
        .iter()
        .copied()
        .skip(offset)
    ...
    let first = (offset + 1).min(CRAFTING_RECIPES.len());
    let last = (offset + CRAFTING_VISIBLE_RECIPES).min(CRAFTING_RECIPES.len());
    let label = format!(
        "RECIPES {first}-{last} / {}  •  SCROLL",

use super::{
    ChunkPos, FurnitureConfiguration, FurnitureDefinition, FurnitureFacing, FurnitureInteraction,
    FurnitureObject, FurnitureSupport, LASER_BORE_MAX_LENGTH, LASER_DRILL_MAX_LENGTH,
    LaserBoreBeam, LaserDrillAim, LaserDrillBeam, Layer, LiftStationConfiguration, ObjectId,
    ObjectPlacementError, ObjectTypeId, POWERED_CABLE_OBJECT, RED_SHAFT_BORE_WIDTH, ROPE_OBJECT,
    RedShaftBoreBeam, RemovedObject, SKY_MACHINE_MIN_ELEVATION_DECIMETRES, TargetPriority, TileId,
    TilePos, World, WorldObject, configuration_variant, decoration_definition,
...
impl World {
    ...
    pub fn laser_bore_target(&self, object: ObjectId, powered: bool) -> Option<TilePos> {
        ...
        match object.object_type {
            FurnitureObject::LASER_BORE => self.laser_bore_beam(object, powered)?.target,
            FurnitureObject::RED_SHAFT_BORE => {
                let beam = self.red_shaft_bore_beam(object, powered)?;
                beam.target_y
                    .map(|y| TilePos::new(beam.first_x + beam.width / 2, y))
            }
            FurnitureObject::LASER_DRILL => self.laser_drill_beam(object, powered)?.target,
            _ => None,
        }
    }
    ...
    pub(crate) fn laser_bore_beam(
        ...
        let length_limit = LASER_BORE_MAX_LENGTH
    ...
    pub(crate) fn red_shaft_bore_beam(
        ...
        if object.object_type != FurnitureObject::RED_SHAFT_BORE || !object.active || !powered {
            return None;
        }
        let first_x = object.anchor.x + 1;
        let first_y = object.anchor.y + u32::from(object.height);
        let length_limit = LASER_BORE_MAX_LENGTH
            .saturating_add(self.specialist_bonuses().drill_depth_tiles())
            .min(self.height.saturating_sub(first_y));
        let target_y = (0..length_limit).find_map(|offset| {
            let y = first_y + offset;
            (first_x..first_x + RED_SHAFT_BORE_WIDTH)
                .any(|x| self.tile_in_bounds(x, y, Layer::Foreground) != TileId::EMPTY)
                .then_some(y)
        });
        Some(RedShaftBoreBeam {
            first_x,
            width: RED_SHAFT_BORE_WIDTH,
            first_y,
            length_tiles: target_y.map_or(length_limit, |y| y - first_y),
            target_y,
        })
    }
    ...
    pub(crate) fn laser_drill_beam(
        ...
        if object.object_type != FurnitureObject::LASER_DRILL || !object.active || !powered {
            return None;
        }
...
pub(crate) const fn is_turret_type(object_type: ObjectTypeId) -> bool {
    matches!(
        object_type,
